...
def main():
    ...
    # 创建光学系统
    # mesh=1024*10+1 的大网格在 8GB 显存下 complex128 会 OOM，显式降为 complex64
    optical_system = OpticalSystem(wavelength, x, y, initial_field, dtype=cp.complex64)

    # # 添加光学元件
    optical_system.add_element(MomentumSpacePlate(z_position=0, modulation_function=lambda KX, KY: cp.exp(1j * 2 * cp.arctan2(KY, KX))))
